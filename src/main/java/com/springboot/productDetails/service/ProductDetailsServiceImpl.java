...
public class ProductDetailsServiceImpl implements ProductDetailsService {
	...
	private Response transformResponse(Response response, String labelType) {
		...
		transformedResponse.setProducts(getProcessableProducts(response.getProducts(), labelType));
		return transformedResponse;
	}
	
	private Products[] getProcessableProducts(Products[] products, String labelType)
	{
		...
		for (Products product : products) {
			...
			if(price !=null)
			{
				if((price.getWas() == null || StringUtils.isEmpty(price.getWas()))
						|| (price.getNow() == null || StringUtils.isEmpty(price.getWas())))
				{
					continue;
				}
			}
			...
			// populate PriceLabel
			populatePriceLabel(product, (labelType == null? "ShowWasNow": labelType));
		...
		Products [] productArray = productsList.toArray(new Products[productsList.size()]);
	...
	private void transformColorSwatches(ColorSwatches[] colorSwatchesArray) {
		for (ColorSwatches colorSwatches : colorSwatchesArray) {
			colorSwatches.setRgbColor((productUtils.getRGB(colorSwatches
					.getBasicColor())));
		}
	}
	...
	public void populatePriceLabel(Products product, String labelType) {
		...
		switch (labelType) {
			...
			case "ShowPercDscount":
				...
				builder.append(Math.round(percentage))
				.append("% off - now ")
	...
	private String getFormattedPrice(String price) {
		...
		if (price != null && price.length() != 0) {
			...
			String formattedPrice = ProductDetailsConstants.getNumberFormat("GDP",
					maxFractionDigits).format(new Double(nowPrice));
